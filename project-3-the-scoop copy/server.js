...
//Creates a new comment and adds it to the database, returns 201 response with comment on comment property of response body
function createComment(url, request){
  const requestComment = request.body && request.body.comment;
  const response = {};

  if (requestComment && requestComment.body && requestComment.username &&
    database.users[requestComment.username] && requestComment.articleId && database.articles[requestComment.articleId]){
      const comment = {
  ...
  //add the new comment to the article's list of comment IDs
  database.articles[comment.articleId].commentIds.push(comment.id);
  //add the new comment to the author's list of comments
...
//Update Comments. Recieves comment ID from url parameter and updated comment from comment property of request body
//Updates body of the corresponding comment in the database and returns a 200 response with the updated comment on the comment property of the response body
...
//Delete Comment. Receives comment ID from url parameter and deletes the comment from the database
function deleteComment(url, request){
  ...
  if (savedComment){
      ...
      //remove the deleted comment id from the article it is on
      ...
      //remove the deleted comment id from the user
...
function upvoteComment(url, request){
  ...
  if (savedComment && username && database.users[username]){
...
//function loadDatabase: Reads a YAML file containing the database and returns a JavaScript object representing the database
function loadDatabase(yamlFile){
  const database = yaml.safeLoad(yamlFile);

  return database;

}

//function saveDatabase: Writes the current value of the database to a YAML filter
